...
function buildTripHtml(){
  const name=escapeHtml($("tripName").value.trim());
  const date=$("tripDate").value;
  const desc=escapeHtml($("tripDescription").value.trim());
  const dateText=date?new Date(date+"T12:00:00").toLocaleDateString("es-ES",{day:"numeric",month:"long",year:"numeric"}):"";
  const placeHtml=places.filter(p=>p.name.trim()).map(p=>`<article class="place-card"><span>✦</span><h3>${escapeHtml(p.name)}</h3><p>${escapeHtml(p.description||"")}</p></article>`).join("");
